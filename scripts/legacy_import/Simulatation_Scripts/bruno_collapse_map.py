...
# Initialize fresh matrix
energy_matrix = np.zeros((len(kappa_vals), len(T_vals)))

# Dynamically calculate entropy and energy for each (κ, T)
for i, kappa in enumerate(kappa_vals):
    for j, T in enumerate(T_vals):
        try:
            S_proj = (4 / 3) * a * T**3 / kappa
            T_calc = (S_proj * kappa) / (4 * a)
            T_clipped = min(T_calc, 1e13)
            u = a * T_clipped**4
            energy_matrix[i, j] = u * shell_volume / 1e-7  # convert to erg
        except Exception as e:
            print(f"Error at κ={kappa}, T={T}: {e}")
...
plt.tight_layout()
plt.savefig("bruno_flash_map_fixed.png", dpi=300)
plt.show()
